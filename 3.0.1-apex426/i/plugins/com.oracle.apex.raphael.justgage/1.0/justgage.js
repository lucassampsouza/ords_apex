 ...
 * -----------------------------
 ...
 JustGage = function(config) {
  ...
  obj.config =
  {
    ...
    // custom sectors for colors
    customSectors : (config.customSectors) ? config.customSectors : [],

    // markers : [] of objects
    // markers to be rendered on the gauge
    markers : (config.markers) ? config.markers : []
  };
  ...
  // path represents a discrete marker, not a gauge section
  obj.canvas.customAttributes.mark = function (value, min, max, w, h, dx, dy, gws, donut) {

    var alpha, Ro, Ri, Cx, Cy, Xi, Yi, path;
  ...
  // marker
  obj.markers = [];
  for(var i in obj.config.markers)
  {
    obj.markers.push(obj.canvas.path().attr({
      "stroke": obj.config.markers[i].color,
      "stroke-width": 2,
      mark: [
        obj.config.markers[i].value,
        obj.config.min,
        obj.config.max,
        obj.params.widgetW,
        obj.params.widgetH,
    ...
    if(obj.config.donut) {
      obj.markers[i].transform("r" + obj.config.donutStartAngle + ", " + (obj.params.widgetW/2 + obj.params.dx) + ", " + (obj.params.widgetH/1.95 + obj.params.dy));
    }
  }
...
JustGage.prototype.generateShadow = function(svg, defs) {
  ...
  var obj = this;
  var gaussFilter, feOffset, feGaussianBlur, feComposite1, feFlood, feComposite2, feComposite3, uShadowId;

  uShadowId = "inner-shadow";
  var suffix = 1;
  while ( document.getElementById( uShadowId ) ) {
    suffix += Math.floor( Math.random() * 100 );
    uShadowId = "inner-shadow-" + ( suffix );
  }

  // FILTER
  gaussFilter = document.createElementNS(svg,"filter");
  gaussFilter.setAttribute("id", uShadowId);
  ...
  if (!obj.config.hideInnerShadow) {
    obj.canvas.canvas.childNodes[2].setAttribute("filter", "url(#" + uShadowId + ")");
    obj.canvas.canvas.childNodes[3].setAttribute("filter", "url(#" + uShadowId + ")");
  }
