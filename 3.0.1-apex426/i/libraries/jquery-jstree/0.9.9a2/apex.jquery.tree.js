	...
	function tree_component () {
		return {
			...
			init : function(elem, conf) {
				...
				// THEMES
				this.container.addClass("tree clearfix");
				if(this.settings.ui.theme_name !== false) {
					if(this.settings.ui.theme_path === false) {
						$("script").each(function () { 
							if(this.src.toString().match(/apex\.jquery\.tree.*?js$/)) { _this.settings.ui.theme_path = this.src.toString().replace(/apex\.jquery\.tree.*?js$/, "") + "themes/" + _this.settings.ui.theme_name + "/style.css"; return false; }
						});
					}
				...
				if((typeof new_name).toLowerCase() == "string") {
					...
					_this.inp
						...
						.bind("keydown",		function (event) { 
								...
								if(key == 27) { this.value = last_value; this.blur(); return false; }
								if(key == 13) { this.blur(); return false; }
							});
	...
	tree_component.mouseup = function(event) {
		...
		if(tmp.foreign === false && tmp.drag_node && tmp.drag_node.size()) {
			...
			if(tmp.move_type) {
				...
				if(tree1) {
                    tree1.moved(tmp.dragged, tmp.ref_node, tmp.move_type, false, (tmp.origin_tree.settings.rules.drag_copy == "on" || (tmp.origin_tree.settings.rules.drag_copy == "ctrl" && ( event.ctrlKey || event.altKey ))))
                }
			}
			tmp.move_type	= false;
			tmp.ref_node	= false;
	...
	$.extend($.tree.datastores, {
		...
		"json" : function () {
			return {
					...
					if(tree.settings.languages.length) {
						for(var i = 0; i < tree.settings.languages.length; i++) {
							...
							if(data.data[tree.settings.languages[i]] && data.data[tree.settings.languages[i]].icon) {
								str += "<ins style='background-image:url(\"" + data.data[tree.settings.languages[i]].icon + "\");'>&nbsp;</ins>";
							}
							else str += "<ins>&nbsp;</ins>";
						...
						str += ">";
						if(data.data.icon) {
							str += "<ins style='background-image:url(\"" + data.data.icon + "\");'>&nbsp;</ins>";
						}
						else str += "<ins>&nbsp;</ins>";
				...
				load	: function(data, tree, opts, callback) {
                    var staticData = opts["static"];

					if( staticData ) {
                        if ( $.isFunction( staticData )) {
                            staticData = staticData.call();
                        }
                        callback.call(null, staticData);
					} else {
						$.ajax({
							'type'		: opts.method,
							'url'		: opts.url, 
...
/*
 * jsTree contextmenu plug-in (Oracle modified version with some enhancements)
 *
 */
(function ($) {
    $.extend($.tree.plugins, {
        "contextmenu" : {
            ...
            show : function(obj, t) {
                ...
                for(var i in opts.items) {
                    ...
                    var r = 1;
          /* if visible is not a function and it's an array of valid node types. Check them against the current node */
          if(typeof opts.items[i].visible == "object") r = ($.inArray(t.get_type($.tree.plugins.contextmenu.data.a), $.makeArray(opts.items[i].visible)) == -1)?-1:1;
                    if(typeof opts.items[i].visible == "function") r = opts.items[i].visible.call(null, $.tree.plugins.contextmenu.data.a, t);
                    if(r == -1) continue;
                    else cnt ++;
          if(opts.items[i].is_separator === true) {
