...
tree.Comment.prototype = {
    ...
    eval: function () { 
        ...
        if(this.value.indexOf("\"var\"") > -1){
            /*var variable = null;
            var properties = {}; 
            var propertyStrings = this.value.match(new RegExp("\\$[^:]+:\\s*[^;]+;", "g"));
            var propertyStringArray = null;
            
            for(propertyString in propertyStrings){
                propertyStringArray = propertyStrings[propertyString].split(":");
                propertyStringArray[0] = propertyStringArray[0].replace(new RegExp("^\\$"), "").replace(new RegExp(":$"), "");
                propertyStringArray[1] = propertyStringArray[1].replace(new RegExp("^\\s+"), "").replace(new RegExp(";$"), "");
                properties[propertyStringArray[0]] = propertyStringArray[1];
            }
